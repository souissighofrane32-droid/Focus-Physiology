...
const FALLBACK_QUESTIONS: QuestQuestion[] = [
  {
  ...
  },
  {
    ...
    a: "-70 mV",
    options: ["-70 mV", "0 mV", "+30 mV", "-90 mV"],
  },
];
...
export function localQuestFromNotes(notes: string): Quest {
  ...
  const questions: QuestQuestion[] =
    sentences.length >= 4
      ? sentences.slice(0, 6).map((s, i) => {
          ...
          const blank = s.replace(new RegExp(key, "i"), "______");
          return {
            q: `From your notes: ${blank}`,
            a: key,
            options: shuffle([
              key,
              words[0] ?? "tissue",
...
export const generateQuest = createServerFn({ method: "POST" })
  .validator((input: { notes: string }) => input)
  .handler(async ({ data }): Promise<{ ok: true; quest: Quest } | { ok: false; error: string; quest: Quest }> => {
    const notes = (data.notes ?? "").slice(0, 8000);
    const fallback = localQuestFromNotes(notes);
    const apiKey = process.env.XAI_API_KEY;
    if (!apiKey || notes.trim().length < 12) {
      return { ok: true, quest: fallback };
    }
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You write cozy farm-fantasy study stories. Return ONLY JSON with keys title, chapters (array of {title,text}), questions (array of {q,a,options} with exactly 4 options, a must be one of options). 4 short chapters. 6 questions drawn from the notes. No markdown.",
            },
            {
              role: "user",
      ...
      if (!res.ok) return { ok: false, error: `AI ${res.status}`, quest: fallback };
      const body = (await res.json()) as {
        choices?: { message?: { content?: string } }[];
      };
      const raw = body.choices?.[0]?.message?.content ?? "";
      const parsed = parseJsonObject(raw);
      if (!parsed) return { ok: true, quest: fallback };

      const chapters = Array.isArray(parsed.chapters)
        ? (parsed.chapters as QuestChapter[]).filter((c) => c?.title && c?.text).slice(0, 6)
        : fallback.chapters;
      const questions = Array.isArray(parsed.questions)
        ? (parsed.questions as QuestQuestion[])
            .filter((q) => q?.q && q?.a && Array.isArray(q.options) && q.options.length >= 2)
            .slice(0, 8)
        : fallback.questions;
      ...
      return { ok: false, error: "AI unavailable", quest: fallback };
    }
  });

export const extractNotesFromImage = createServerFn({ method: "POST" })
  .validator((input: { imageDataUrl: string }) => input)
  .handler(async ({ data }): Promise<{ ok: true; text: string } | { ok: false; error: string }> => {
    const apiKey = process.env.XAI_API_KEY;
    if (!apiKey) return { ok: false, error: "Camera reading needs AI in this environment." };
    const url = data.imageDataUrl.slice(0, 1_200_000);
    try {
      const res = await fetch("https://api.x.ai/v1/chat/completions", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) return { ok: false, error: `Could not read the photo (${res.status}).` };
      const body = (await res.json()) as {
        choices?: { message?: { content?: string } }[];
      };
      const text = body.choices?.[0]?.message?.content?.trim() ?? "";
      if (!text) return { ok: false, error: "No text found in the photo." };
      return { ok: true, text };
    } catch {
